...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!employeeId || !newRole) {
      return new Response(JSON.stringify({ error: "Missing employeeId or newRole" }), {
    ...
    if (employee.role === 'admin' && newRole !== 'admin') {
      return new Response(JSON.stringify({ error: "Cannot change role of another admin" }), {
        status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
